...
def compute_framing_signals(text: str) -> dict[str, str]:
    ...
    sensational_count = sum(
        len(re.findall(rf"\b{w}\b", text_lower)) for w in sensational_words
    )
    ...
    attr_density = (
        "High"
        if (attr_count / words_total) > 0.01
        else "Moderate"
        if (attr_count / words_total) > 0.003
        else "Low"
    )
    sensational_level = (
        "High"
...
def analyze_article():
    ...
    try:
        key_points = keypoint_extractor.extract_key_points(
            cleaned_text, top_n=req.num_key_points
        )
    except Exception as e:  # noqa: BLE001
