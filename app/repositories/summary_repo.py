...
class SummaryRepository:
    ...
    def get_by_article_and_profile(
        self, article_id: str, length_profile: str
    ) -> Optional[Summary]:
        return (
            self.session.query(Summary)
            .filter(
                Summary.article_id == article_id,
                Summary.length_profile == length_profile,
            )
            .first()
        )
    ...
    def get_analysis_by_article_id(self, article_id: str) -> Optional[AnalysisResult]:
        return (
            ...
            .filter(AnalysisResult.article_id == article_id)
            .first()
        )
    ...
    def get_translation(
        self, summary_id: str, target_language: str
