...
class PDFReportGenerator:
    ...
    @classmethod
    def generate_report(cls, data: Dict[str, Any], target_language: str = "en") -> bytes:
        buffer = io.BytesIO()
        ...
        sub_header_style = ParagraphStyle(
            "SubHeader",
            parent=styles["Normal"],
            fontName="GeneralUnicode" if "GeneralUnicode" in styles["Normal"].fontName else body_font,
            fontSize=8,
            leading=11,
        ...
        sub_text = Paragraph(f"Generated: {now_str} • Pipeline: V2.1 Production", sub_header_style)

        header_table = Table(
            [[header_text, Paragraph("<font color='#047857'><b>VERIFIED ANALYSIS</b></font>", sub_header_style)],
             [sub_text, ""]],
            colWidths=[400, 140],
        )
        ...
        meta_rows = [
            [
                Paragraph(f"<b>Source:</b> {data.get('publisher') or 'Web Publication'}", meta_style),
                Paragraph(f"<b>Language:</b> {data.get('language_name') or target_language.upper()}", meta_style),
                Paragraph(f"<b>Reading Time:</b> {data.get('reading_time') or '~3 min read'}", meta_style),
            ],
            [
                Paragraph(f"<b>Author:</b> {data.get('author') or 'Not available'}", meta_style),
                Paragraph(f"<b>Published:</b> {data.get('published_date') or 'Recent'}", meta_style),
                Paragraph(f"<b>Word Count:</b> {data.get('word_count', 'N/A')}", meta_style),
            ]
        ]
        ...
        if key_points:
            elements.append(Paragraph("<b>KEY POINTS (MMR RANKED)</b>", section_heading_style))
            elements.append(Spacer(1, 2))
            kp_rows = []
        ...
        dominant = sentiment.get("label", "neutral") if isinstance(sentiment, dict) else "neutral"
        ...
        analysis_grid = [
            ...
            [
                Paragraph(f"Dominant Polarity: <b>{dominant.upper()}</b><br/>Positive: {pos}% | Neutral: {neu}% | Negative: {neg}%", body_style),
                Paragraph(f"Original Words: <b>{data.get('word_count', 'N/A')}</b> | Summary Words: <b>{data.get('summary_word_count', 'N/A')}</b><br/>Compression Ratio: <b>{ratio}%</b> | Processing Latency: <b>{lat_sec}</b>", body_style),
            ]
        ]
        ...
        if entities or keywords:
            ent_list = [f"{e.get('text')} ({e.get('label')})" for e in entities[:6]] if entities else ["None detected"]
            kw_list = [f"#{k}" for k in keywords[:6]] if keywords else ["None detected"]

            tags_grid = [
                [Paragraph("<b>NAMED ENTITIES</b>", section_heading_style), Paragraph("<b>SALIENT KEYWORDS</b>", section_heading_style)],
                [Paragraph(", ".join(ent_list), body_style), Paragraph("  ".join(kw_list), body_style)]
            ]
