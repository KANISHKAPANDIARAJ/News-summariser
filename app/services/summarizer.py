...
class HierarchicalSummarizer:
    def __init__(self, chunk_tokens: int = 750):
        ...
        self.chunker = SentenceAwareChunker(max_tokens=chunk_tokens, overlap_sentences=1)
    ...
    def summarize(
        ...
        if not text or len(text.strip()) < 30:
            raise SummarizationError("Article content is too short for meaningful summarization.")
        ...
        if len(chunks) == 1:
            logger.info(f"Summarizing single-chunk document ({chunks[0]['token_count']} tokens)")
            final_summary = self._generate_chunk_summary(chunks[0]["text"], min_length, max_length)
        else:
            # Map Phase: Summarize each chunk
            logger.info(f"Executing Map Phase on {len(chunks)} chunks for long document...")
            for idx, chunk in enumerate(chunks):
                ...
                chunk_summ = self._generate_chunk_summary(chunk["text"], chunk_min, chunk_max)
                intermediate_summaries.append(chunk_summ)
                logger.debug(f"Chunk {idx + 1}/{len(chunks)} summarized: {len(chunk_summ)} chars")

            # Reduce Phase: Combine intermediate summaries
            combined_text = " ".join(intermediate_summaries)
            logger.info(f"Executing Reduce Phase on combined intermediate summaries ({len(combined_text)} chars)...")
            ...
            if combined_tokens > 750:
                ...
                second_chunks = self.chunker.chunk_document(combined_text, tokenizer=tokenizer)
                second_summaries = [
            ...
            # Final Synthesis
            final_summary = self._generate_chunk_summary(combined_text, min_length, max_length)
